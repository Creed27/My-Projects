...
steady_clock::time_point solution_time;
unsigned long long max_sol_iter;
...
struct HelpMe
{
    ...
    auto kill_workers() -> void
    {
        ...
        threads.clear();
        std::ofstream writer("/kaggle/working/RRSplit_output_16_bug.csv", std::ios::app);
        if (! times.empty()) {
...
int partition_right(vector<int>& all_vv, int start, int len, const vector<unsigned int> & adjrow, vector<int> &index_right) {
    int i=0;
    for (int j=0; j<len; j++) {
        if (adjrow[all_vv[start+j]]) {
...
int partition_sparse(vector<int>& all_vv, int start, int len, int degree, const ui * adjlist, vector<int> &index_right){
    int pos; int j=0;
...
vector<Bidomain> filter_domains(const vector<Bidomain> & d, vector<int> & left,
        ...
        vector<int> &index_right, bool &best_match)
{
    ...
    for (const Bidomain &old_bd : d) {
        ...
        //right_len = partition(right, r, old_bd.right_len, g1.adjmat[w]);
...
void solve_nopar(const unsigned depth, const Graph & g0, const Graph & g1,
        ...
        unsigned long long & my_thread_nodes, vector<int> &index_right, int inc_size)
{
    if (abort_due_to_timeout)
        return;
    my_thread_nodes++;
    if (my_incumbent.size() < current.size()) {
        inc_size = my_incumbent.size();
    ...
    int w = -1, idx = -1;
    //inizio aggiunta RR
    for(int i = inc_size; i < current.size(); i++){
        if(EqClass[current[i].v] == EqClass[v] && w < current[i].w) w = current[i].w;
    }
    ...
    if(w>0){
        ...
        if(bd.left_len<=bd.right_len && count_left>count_right){
            if(bound+count_right-count_left<=global_incumbent.value){
                cuts++;
                return;
            }
        }
        if(bd.left_len>bd.right_len && (bd.right_len-count_right)>(bd.left_len-count_left)){
            if(bound+(bd.left_len-count_left)-(bd.right_len-count_right)<=global_incumbent.value){
                cuts++;
                return;
            }
        }
    ...
    for (int i=bd.right_len; i>=0; --i) {
        ...
        auto new_domains = filter_domains(domains, left, right, g0, g1, v, w, index_right, best_match);
        current.push_back(VtxPair(v, w));
        solve_nopar(depth + 1, g0, g1, global_incumbent, my_incumbent, current, new_domains, left, right, matching_size_goal, my_thread_nodes, index_right, inc_size);
        current.pop_back();
        ...
        solve_nopar(depth + 1, g0, g1, global_incumbent, my_incumbent, current, domains, left, right, matching_size_goal, my_thread_nodes, index_right, inc_size);
...
void solve(const unsigned depth, const Graph & g0, const Graph & g1,
        ...
        vector<VtxPair> & current, vector<Bidomain> & domains,
        vector<int> & left, vector<int> & right, const unsigned int matching_size_goal,
        const Position & position, HelpMe & help_me, unsigned long long & my_thread_nodes, vector<int> &index_right, int inc_size)
{
    if (abort_due_to_timeout)
        return;
    my_thread_nodes++;
    if (per_thread_incumbents.find(std::this_thread::get_id())->second.size() < current.size()) {
        inc_size = per_thread_incumbents.find(std::this_thread::get_id())->second.size();
    ...
    std::function<void (unsigned long long &)> helper_function = [&shared_i, &g0, &g1, &global_incumbent, &per_thread_incumbents, &position, &depth,
        i_end, matching_size_goal, current, domains, left, right, &help_me, &bound, index_right, inc_size] (unsigned long long & help_thread_nodes) {
        ...
        unsigned int help_bound = bound;
        vector<int> help_index_right = index_right;
        ...
        int help_w = -1;
        //aggiunta RR
        for(int i = inc_size; i < help_current.size(); i++){
            if(EqClass[help_current[i].v] == EqClass[help_v] && help_w < help_current[i].w) help_w = help_current[i].w;
        }
        /*for (VtxPair & a:help_current){
            if(EqClass[a.v]==EqClass[help_v] && help_w < a.w) help_w = a.w;
        }*/
        if(help_w>0){
            int count_left=0, count_right=0;
            for (int i=help_bd.left_len; i>=0; --i) if(EqClass[help_left[help_bd.l+i]]==EqClass[help_v]) count_left++;
            for (int i=(help_bd.right_len+1); i>=0; --i) if(help_right[help_bd.r+i]>help_w) count_right++;
            if(help_bd.left_len<=(help_bd.right_len+1) && count_left>count_right){
                if(help_bound+count_right-count_left<=per_thread_incumbents.size()){
                    cuts++;
                    return;
                }
            }
            if(help_bd.left_len>(1+help_bd.right_len) && (help_bd.right_len-count_right)>(help_bd.left_len-count_left)){
                if(help_bound+(help_bd.left_len-count_left)-((help_bd.right_len+1)-count_right)<=per_thread_incumbents.size()){
                    cuts++;
                    return;
                }
            }
        ...
        int flag = 1;
        bool best_match = false;
        for (int i = 0 ; i < i_end && flag; i++) {
            int idx = index_of_next_smallest(help_right, help_bd.r, help_bd.right_len+1, help_w);
            if(idx == -1) flag = 0;
            if (i != i_end - 1 && idx != -1) {
                help_w = help_right[help_bd.r + idx];
                //aggiunta swap
                std::swap(help_index_right[help_w],help_index_right[help_right[help_bd.r + help_bd.right_len]]);
                ...
                if (i == which_i_should_i_run_next) {
                    ...
                    auto new_domains = filter_domains(help_domains, help_left, help_right, g0, g1, help_v, help_w,
                             help_index_right, best_match);
                    help_current.push_back(VtxPair(help_v, help_w));
                    if (depth > split_levels) {
                        solve_nopar(depth + 1, g0, g1, global_incumbent, per_thread_incumbents.find(std::this_thread::get_id())->second, help_current, new_domains, help_left, help_right, matching_size_goal, help_thread_nodes, help_index_right, inc_size);
                    }
                    else {
                        auto new_position = position;
                        new_position.add(depth, i + 1);
                        solve(depth + 1, g0, g1, global_incumbent, per_thread_incumbents, help_current, new_domains, help_left, help_right, matching_size_goal, new_position, help_me, help_thread_nodes, help_index_right, inc_size);
                    }
                ...
                help_bd.right_len++;
                //aggiunta loop finale RR
                for(int j = 0; j<help_bd.left_len; ++j){
                    if(EqClass[help_left[help_bd.l+j]]==EqClass[help_v]){
                        std::swap(help_left[help_bd.l+j], help_left[help_bd.l+help_bd.left_len-1]);
                        --help_bd.left_len; --j;
                    }
                }
                ...
                if (i == which_i_should_i_run_next) {
                    which_i_should_i_run_next = shared_i++;
                    if (depth > split_levels) {
                        solve_nopar(depth + 1, g0, g1, global_incumbent, per_thread_incumbents.find(std::this_thread::get_id())->second, help_current, help_domains, help_left, help_right, matching_size_goal, help_thread_nodes, help_index_right, inc_size);
                    }
                    else {
                        auto new_position = position;
                        new_position.add(depth, i + 1);
                        solve(depth + 1, g0, g1, global_incumbent, per_thread_incumbents, help_current, help_domains, help_left, help_right, matching_size_goal, new_position, help_me, help_thread_nodes, help_index_right, inc_size);
                    }
                }
            }
        }
    ...
    std::function<void (unsigned long long &)> main_function = [&] (unsigned long long & main_thread_nodes) {
        ...
        if(w>0){
            int count_left=0, count_right=0;
            for (int i=bd.left_len; i>=0; --i) if(EqClass[left[bd.l+i]]==EqClass[v]) count_left++;
            for (int i=(1+bd.right_len); i>=0; --i) if(right[bd.r+i]>w) count_right++;
            if(bd.left_len<=(1+bd.right_len) && count_left>count_right){
                if(bound+count_right-count_left<=global_incumbent.value){
                    cuts++;
                    return;
                }
            }
            if(bd.left_len>(1+bd.right_len) && (bd.right_len+1-count_right)>(bd.left_len-count_left)){
                if(bound+(bd.left_len-count_left)-(bd.right_len+1-count_right)<=global_incumbent.value){
                    cuts++;                    
                    return;
                }
            }
            ...
            if (i != i_end - 1 && idx != -1) {
                ...
                if (i == which_i_should_i_run_next) {
                    ...
                    auto new_domains = filter_domains(domains, left, right, g0, g1, v, w,
                            index_right, best_match);
                    current.push_back(VtxPair(v, w));
                    if (depth > split_levels) {
                        solve_nopar(depth + 1, g0, g1, global_incumbent, per_thread_incumbents.find(std::this_thread::get_id())->second, current, new_domains, left, right, matching_size_goal, main_thread_nodes, index_right, inc_size);
                    }
                    else {
                        auto new_position = position;
                        new_position.add(depth, i + 1);
                        solve(depth + 1, g0, g1, global_incumbent, per_thread_incumbents, current, new_domains, left, right, matching_size_goal, new_position, help_me, main_thread_nodes, index_right, inc_size);
                    }
                    current.pop_back();
                    if(best_match) return;
                }
                ...
                for(int j = 0; j<bd.left_len; ++j){
                    if(EqClass[left[bd.l+j]]==EqClass[v]){
                        std::swap(left[bd.l+j], left[bd.l+bd.left_len-1]);
                        --bd.left_len; --j;
                    }
                }
                if (bd.left_len == 0)
                ...
                if (i == which_i_should_i_run_next) {
                    which_i_should_i_run_next = shared_i++;
                    if (depth > split_levels) {
                        solve_nopar(depth + 1, g0, g1, global_incumbent, per_thread_incumbents.find(std::this_thread::get_id())->second, current, domains, left, right, matching_size_goal, main_thread_nodes, index_right, inc_size);
                    }
                    else {
                        auto new_position = position;
                        new_position.add(depth, i + 1);
                        solve(depth + 1, g0, g1, global_incumbent, per_thread_incumbents, current, domains, left, right, matching_size_goal, new_position, help_me, main_thread_nodes, index_right, inc_size);
                    }
                }
            }
        }
...
std::pair<vector<VtxPair>, unsigned long long> mcs(const Graph & g0, const Graph & g1, vector<int> &index_right) {
    ...
    if (arguments.big_first) {
        for (int k=0; k<g0.n; k++) {
            ...
            solve(0, g0, g1, global_incumbent, per_thread_incumbents, current, domains_copy, left_copy, right_copy, goal, position, help_me, global_nodes, index_right, inc_size);
        ...
        for (auto & t : help_me.threads)
            per_thread_incumbents.emplace(t.get_id(), vector<VtxPair>());
        solve(0, g0, g1, global_incumbent, per_thread_incumbents, current, domains, left, right, 1, position, help_me, global_nodes, index_right, inc_size);
        help_me.kill_workers();
        for (auto & n : help_me.nodes)
            global_nodes += n;
        for (auto & i : per_thread_incumbents)
...
int main(int argc, char** argv) {
    ...
    //std::ofstream writer("/kaggle/working/RRSplit_output_2.csv", std::ios::app);
    //writer << "thread_work_times,filename1,filename2,elapsedtime(ms),solution_size,nodes\n";
    //writer.close();
    ...
    std::pair<vector<VtxPair>, unsigned long long> solution = mcs(g0_sorted, g1_sorted, index_right);
    ...
    auto max_sol_time = duration_cast<std::chrono::milliseconds>(solution_time - start).count();
    ...
    //cout << arguments.filename1 << " " << arguments.filename2 << endl;
    if (!check_sol(g0, g1, solution.first))
        fail("*** Error: Invalid solution\n");
    /*cout << "Max_sol_time:" << max_sol_time << std::endl;
    cout << "Max_sol_iter:" << max_sol_iter << std::endl;
    cout << "Cuts:" << cuts << std::endl;
    ...
    //cout << endl;
    /*for(auto &it :index_right)
        cout << it << " ";*/
    //g0 deve essere più piccolo di g1 nell'algoritmo
    int solution_size = solution.first.size();
    unsigned long long nodes = solution.second;
    std::ofstream writer2("/kaggle/working/RRSplit_output_16_bug.csv", std::ios::app);
    writer2 << "," << arguments.filename1 << "," << arguments.filename2 << "," << time_elapsed << "," << solution_size << "," << nodes << "," << max_sol_iter << "," << max_sol_time << "," << cuts << std::endl; 
    writer2.close(); 
}           
